...
function testUDP() {
	...
	t.open().then(function(){
		...
		t.write(randomData(100, 'setFloat64')).then(function(bytesWritten){
			...
			t.read(t.bytesWritten).then(function(){
				var str = randomString(100, true);
				console.info("Str [%s]", str);
				t.write(str).then(function(){
					t.close();
...
function randomString(maxLength, noEol) {
	...
	var arr = new Array(x);
	for(var i = 0; i < x; i++) {
		var ch = (Math.round((Math.random()*100))%106) + 21;
		while(noEol && ch==10) {
			ch = (Math.round((Math.random()*100))%106) + 21;
		}
		arr[i] = Math.abs(ch);
	}	
...
function hbaseTest() {
	// 10.12.114.42:9090
	var t = new chrome.Thrift.Transport("tcp://10.12.114.42:9090");
	t.open().then(function(){
		console.info("TCP Connection Opened");
		var inp = new chrome.Thrift.TBinaryProtocol(t, true, true, {});
		var out = new chrome.Thrift.TBinaryProtocol(t, true, true, {});
		window.HbaseClient = new HbaseClient(inp, out);
		console.info("HBase Client: [%O]", window.HbaseClient);
		HbaseClient.getTableNames(function(err, data) { console.info("getTableNames Response:%O", arguments); });
	});
}
