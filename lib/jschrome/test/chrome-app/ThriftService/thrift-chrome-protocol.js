...
chrome.Thrift.TBinaryProtocol = function(transport, inp, strictRead, strictWrite, options) {
	if(transport==null) throw new Error("The passed transport was null");
	...
	if(this.inp) {
		...
		// tell transport this is the out
	}
};
...
chrome.Thrift.TBinaryProtocol.prototype = {
	...
	writeMessageBegin: function(name, type, seqid) {
	    if (this.strictWrite) {
	      this.writeI32(chrome.Thrift.Protocol.VERSION_1 | type);
	...
	_readNumber: function(tp) {
		...
		if(this.readableBytes < SZ) {
			throw new Error("Buffer underflow. Request for [" + tp + "] : [" + SZ + "], Available: [" + this.readableBytes + "]");
		}
	...
	flush: function() {
		...
		console.debug("Protocol Flush: [%O]", flushData);
		return flushData;
	},
};
