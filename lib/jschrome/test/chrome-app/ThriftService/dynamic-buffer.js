...
function appendBuffer(source, target, spaceAvail, targetOffset, sourceOffset) {
	...
	var transferSize = 0;
	var transferCount = 0;
	var sourceArrView = null;
	var targetArrView = null;
	if(bytesToCopy%8==0) {
		transferSize = 8;
		transferCount = bytesToCopy / transferSize;
		sourceArrView = new Float64Array(source, sourceOffset, transferCount);
		targetArrView = new Float64Array(target, targetOffset, transferCount);
	} else if(bytesToCopy%4==0) {
		transferSize = 4;
		transferCount = bytesToCopy / transferSize;
		sourceArrView = new Uint32Array(source, sourceOffset, transferCount);
		targetArrView = new Uint32Array(target, targetOffset, transferCount);		
	} else {
		transferSize = 1;
		transferCount = bytesToCopy / transferSize;
		sourceArrView = new Uint8Array(source, sourceOffset, transferCount);
		targetArrView = new Uint8Array(target, targetOffset, transferCount);				
	}
	for(var i = 0; i < transferCount; i++) {
		targetArrView[i] = sourceArrView[i];
	}
	return bytesToCopy;
}
