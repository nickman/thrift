...
chrome.Thrift.Transport.prototype = {
    ...
    flush: function(async) {
        throw new chrome.Thrift.TApplicationException("flush", 12);
    },
    isOpen: function() {
        var self = this;  
        var d = $.Deferred();
    ...
    write: function(data) {
        ...
        deferredData.then(function(arr){
            if(self.socketProtocol.protocolName=='tcp') {
                chrome.socket.write(self.socketId, arr, function _chrome_onwrite_(writeInfo) {
                    console.debug("Write Complete:%o", writeInfo);
                    d.resolve(writeInfo);
                });
            } else if(self.socketProtocol.protocolName=='udp') {
                console.debug("Socket [%s] Sending to [%s]:[%s]", self.socketId, self.socketProtocol.protocolHost, self.socketProtocol.protocolPort);
                chrome.socket.sendTo(self.socketId, arr, self.socketProtocol.protocolHost, self.socketProtocol.protocolPort, function _chrome_onsendto_(writeInfo){
    ...
    initSocket: function() {
        ...
        var self = this;
        var d = $.Deferred();
        chrome.socket.create(self.socketProtocol.protocolName, {}, function(createInfo){
...
chrome.Thrift.TCPTransport.prototype = {
    ...
    open: function() {    
        var self = this;   
            ...
            // This is an attempt to make thew transport re-usable if the underlying socket is closed,
            // but it fails when the sockets is closed independently of this framework, such as when
            //     A. Someone calls chrome.socket.close(mySocketId) or chrome.socket.destroy(mySocketId)
            //     B. The remote side closes the connection
            // ..... because we don't know the socket is closed and the socketId is not set to -1.
            // So far as I can tell, there is no way to auto-detect this.
        // FIXME
        // if(this.socketId==-1) {
        //     this.initSocket().then(function(){
        //         self.open();
        //     });
        // }
        var d = $.Deferred();
