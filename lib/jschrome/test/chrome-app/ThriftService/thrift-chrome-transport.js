...
chrome.Thrift.Transport.prototype = {
    ...
    flush: function(async) {
        console.info("Transport Flush. Caller Data: %O", arguments);
    },
    open: function() {        
        var d = $.Deferred();
        var self = this;
...
window.requireSpace = {Thrift: chrome.Thrift};
...
function require(module) {
    ...
    if(window.requireSpace[module.initCap()]==null) {
        window.requireSpace[module.initCap()]  = {m: module};
        console.debug("RequireSpace [%s]", module.initCap())
    }
    return window.requireSpace;
}
*/
